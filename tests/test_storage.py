...
def test_upsert_group_and_read_back(tmp_path):
    conn = _conn(tmp_path)
...
def test_upsert_preserves_created_but_updates_updated(tmp_path):
    ...
    conn.commit()
    row = storage.get_group_by_path(conn, "c/n.md")
    assert row.title == "N2"
...
def test_replace_entries_wipes_and_reinserts(tmp_path):
    ...
    storage.append_entry(conn, gid, header="one", content="body-1")
    storage.append_entry(conn, gid, header="two", content="body-2")
    conn.commit()

    assert len(storage.list_entries(conn, gid)) == 2
    ...
    storage.replace_entries(conn, gid, new_entries)
    conn.commit()

    entries = storage.list_entries(conn, gid)
...
def test_fts_search_finds_by_header_and_content(tmp_path):
    ...
    storage.append_entry(conn, gid, header="战斗流程", content="出牌 结算 胜负判定")
    storage.append_entry(conn, gid, header="Bug 反馈", content="合服问题 分服 crash")
    conn.commit()
...
def test_delete_group_cascades_and_wipes_fts(tmp_path):
    ...
    storage.append_entry(conn, gid, header="h", content="findable-token")
    conn.commit()

    assert storage.search_fts(conn, "findable-token")

    storage.delete_group(conn, gid)
    conn.commit()

    assert not storage.search_fts(conn, "findable-token")
    assert storage.get_group_by_path(conn, "c/x.md") is None
    conn.close()


def test_comment_marks_group_dirty(tmp_path):
    conn = _conn(tmp_path)
    gid = storage.upsert_group(
        conn, path="c/y.md", category="c", slug="y", title="Y",
        tags=[], dirty=False,
    )
    eid = storage.append_entry(conn, gid, header="h", content="body")
    conn.commit()

    assert storage.get_group_by_path(conn, "c/y.md").dirty is False
    ...
    row = storage.get_group_by_path(conn, "c/y.md")
    assert row.dirty is True

    entry = storage.list_entries(conn, gid)[0]
...
def test_move_entry_to_cold_removes_from_active(tmp_path):
    ...
    eid = storage.append_entry(
        conn, gid, header="stale", content="ancient",
        last_used="2020-01-01T00:00:00+00:00",
    )
    conn.commit()

    batch_id = storage.get_or_create_cold_batch_for_today(conn)
    new_ce = storage.move_entry_to_cold(conn, eid, cold_batch_id=batch_id)
    conn.commit()

    assert new_ce is not None
    # No longer active:
    assert storage.list_entries(conn, gid) == []
